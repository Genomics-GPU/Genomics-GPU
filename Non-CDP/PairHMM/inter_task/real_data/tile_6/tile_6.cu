...
// if(i==1)
  //                      printf("R=%c H=%c  M5=%e I5=%e  D5=%e\n", read_base_4_2.y,read_haplotype_base.y,M5,Il,D5);
	...
	//	printf("time=%d\n",time);
		...
		if(i*6< read_haplotype_number.x && i*6%4!=0)
			read_base_4_1=read_base_array[i*6/4*constant_int[2]];		
	...
	//	file=fopen("less.txt","r");
		...
		fscanf(file,"%d",&size);
		  float * MG;
                ...
                cudaMalloc( (float **)& MG,sizeof(float) *128*60*500*3);
                DG=MG+60*128*500;// ????
                IG=DG+60*128*500;  //?????
