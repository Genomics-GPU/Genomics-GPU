	...
	while(offset<size)
	 {	
		__shared__ float parameter1[768];
		__shared__ float parameter2[768];
		__shared__ float parameter3[768];
		__shared__ float parameter4[768];
		...
		//int time=0;
	//if(threadIdx.x==0)
		for(i=0;i<read_haplotype_number.x/6;i++)
		{	
		...
		{
		int cc=i*6/4*32+32;
		read_base_4_1.x=read_base_4_2.z;
		read_base_4_1.y=read_base_4_2.w;
		char4 read_base_temp;
		read_base_temp=read_base_array[cc];
		read_base_4_1.z=read_base_temp.x;
		read_base_4_1.w=read_base_temp.y;
		read_base_4_2.x=read_base_temp.z;
		read_base_4_2.y=read_base_temp.w;
		}
...
// if(i==1)
  //                      printf("R=%c H=%c  M5=%e I5=%e  D5=%e\n", read_base_4_2.y,read_haplotype_base.y,M5,Il,D5);
	...
	// if(i==1) printf("R=%c H=%c  M6=%e I6=%e  D6=%e\n", read_base_4_2.y,read_haplotype_base.y,M6,Il,D6);
			...
			if(i==read_haplotype_number.x/6-1 && read_haplotype_number.x%6==0)
			{
			result_block=__fadd_rn(result_block,__fadd_rn(M6,Il));
			}
			else
			{
				MG[index]=M6;
	...
	//	printf("time=%d\n",time);
		...
		for(i=i*6;i<read_haplotype_number.x;i++)
                {
                //char4 read_base_4;
                if(i%4==0)
                {
...
int main(int argc, char * argv[])
{
	...
	//	data_prepare+=diff(start,finish);
		...
		while(!feof(file))
		{
			...
			clock_gettime(CLOCK_MONOTONIC_RAW,&start); 
			//32 one chunck.
			int malloc_size_for_each_chunk=(40*4*32+150*4*32*4+125*4*32) ;
 ...
 //cudaDeviceSynchronize();//                       cudaMemcpy(result_h,result_d,size*sizeof(float),cudaMemcpyDeviceToHost);
	...
	//		break;
		...
		printf("fakesize=%d  read_read=%lf  haplotype=%lf\n",fakesize, read_read,haplotype_haplotype);
