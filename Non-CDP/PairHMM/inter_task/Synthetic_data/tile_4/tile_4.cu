	...
	while(offset<size)
	 {	
		__shared__ float parameter1[512];
		__shared__ float parameter2[512];
		__shared__ float parameter3[512];
		__shared__ float parameter4[512];
		...
		// this for loop it increase register number 29
		
		//if number_address.read_number is even
		for(i=0;i<read_haplotype_number.x/4;i++)
		{	
	...
	//	beta=constant[3];
		...
		for(int kk=0;kk<4;kk++)
		{
			...
			float aa=(read_haplotype_base.y==read_base_4.x)? Qm_1:Qm;
		...
		//	beta=1.0-epsion;
			...
			Qm=fdividef(Qm,constant[2]);
			 ...
			 aa=(read_haplotype_base.y==read_base_4.y)?Qm_1:Qm;
			...
			Qm=fdividef(Qm,constant[2]);
			 ...
			 IIMI=__fmul_rn(Il,constant[4]);
			 aa=(read_haplotype_base.y==read_base_4.z)?Qm_1:Qm;
			 MIIDD=__fmul_rn(constant[3], MID);	
		...
		//	beta=1.0-epsion;
			...
			//if(i==3) printf("M3=%e I3=%e  D3=%e\n", M3,I3,D3);
			 MID=__fadd_rn(Il,D3);
			 DDM=__fmul_rn(M4,xiksi);
			 IIMI=__fmul_rn(Il,constant[4]);
			 aa=(read_haplotype_base.y==read_base_4.w)?Qm_1:Qm;
			 MIIDD=__fmul_rn(constant[3], MID);	
			...
			if(i==read_haplotype_number.x/4-1 && read_haplotype_number.x%4==0)
			{
			result_block=__fadd_rn(result_block,__fadd_rn(M4,Il));
			}
			else
			{
				MG[index]=M4;
		...
		for(i=i*4;i<read_haplotype_number.x;i++)
                {
                //char4 read_base_4;
                if(i%4==0)
                {
...
int main(int argc, char * argv[])
{
	...
	//	file=fopen("less.txt","r");
		...
		//			printf("kk=%d  x=%d  y=%d z=%d w=%d \n ",kk,parameter1[kk*32+dd],parameter2[kk*32+dd],parameter3[kk*32+dd],parameter4[kk*32+dd] );
			...
			char4 haplotype_base_data[32*128];
