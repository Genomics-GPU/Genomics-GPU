...
__global__ void cdppack(int const iSize, int iDepth, gasal_gpu_storage_t *gpu_storage,  const uint32_t cdp_actual_n_alns,const uint32_t actual_query_batch_bytes, const uint32_t actual_target_batch_bytes ,int32_t* unpacked_query_batch, uint32_t* unpacked_target_batch, uint32_t *packed_query_batch, uint32_t* packed_target_batch, int query_batch_tasks_per_thread, int target_batch_tasks_per_thread, uint32_t total_query_batch_regs, uint32_t total_target_batch_regs)
{
    uint32_t BLOCKDIM = 128;
    uint32_t N_BLOCKS = (cdp_actual_n_alns + BLOCKDIM - 1) / BLOCKDIM;
  ...
  // uint32_t gpu_storage =  gasal_gpu_storage_t *cdp_gpu_storage;
    int tid = threadIdx.x;
...
void gasal_aln_async(gasal_gpu_storage_t *gpu_storage, const uint32_t actual_query_batch_bytes, const uint32_t actual_target_batch_bytes, const uint32_t actual_n_alns, Parameters *params) {
	...
	if (!(params->isPacked))
	{
		cdppack<<<128,N_BLOCKS>>>(1,1, gpu_storage, (uint32_t)actual_n_alns, actual_query_batch_bytes, actual_target_batch_bytes, (int32_t *)(gpu_storage->unpacked_query_batch),
		(uint32_t*)(gpu_storage->unpacked_target_batch), gpu_storage->packed_query_batch, gpu_storage->packed_target_batch,
		query_batch_tasks_per_thread, target_batch_tasks_per_thread, actual_query_batch_bytes/4, actual_target_batch_bytes/4);
