...
// kernel_createMagic
__global__ void kernel_createWords(int *lengths, long *offsets, char *reads,
unsigned char *magic, int readsCount) {
    int index = threadIdx.x+blockDim.x*blockIdx.x;
    if (index >= readsCount) return;  // 超出范围
    long start = offsets[index];  // 起始位置
    int length = lengths[index];
    long magicStart = index*256;
    if (length < 4) return;
...
void createWords(Data &data, Option &option) {
    ...
    if (wordLength == 4) {
        cudaMallocManaged(&data.magic, readsCount*sizeof(unsigned char)*256);
        cudaMemset(data.magic, 0, readsCount*sizeof(unsigned char)*256);
        kernel_createWords<<<(readsCount+127)/128, 128>>>(data.lengths,
        data.offsets, data.reads, data.magic, readsCount);
        cudaDeviceSynchronize();  // 同步数据
        ...
        std::cout << "生成短词完成" << std::endl;
        // 排序短词
        kernel_sortWords<<<(readsCount+127)/128, 128>>>(data.offsets,
        data.words, data.wordCounts, option.wordLength, readsCount);
        cudaDeviceSynchronize();  // 同步数据
        std::cout << "排序短词完成" << std::endl;
        // 合并相同短词
        cudaMallocManaged(&data.orders, length*sizeof(unsigned short));
        kernel_mergeWords<<<(readsCount+127)/128, 128>>>(data.offsets,
        data.words, data.wordCounts, data.orders, readsCount);
        cudaDeviceSynchronize();  // 同步数据
...
// kernel_magicFilter
__global__ void kernel_magicFilter(unsigned char *magic,int *wordCutoff,
int *jobList, int jobCount, int representative) {
    int index = threadIdx.x+blockDim.x*blockIdx.x;
    if (index >= jobCount) return;  // 超出范围
    int text = representative;
    int query = jobList[index];  // 查询序列的编号
    long textStart = text*256;
    long queryStart = query*256;
    int result = 0;
    for (int i=0; i<256; i++) {
        result += min(magic[textStart+i], magic[queryStart+i]);
    }
    int cutoff = wordCutoff[query];
    if (result < cutoff) {  // 没通过过滤
        jobList[index] = -1;
    }
}
// kernel_dynamic 动态规划
__global__ void kernel_dynamic(float threshold, int *lengths, long *offsets,
...
int *jobList, int jobCount, int representative) {
    ...
    // 批量计算阶段
    int index = threadIdx.x+blockDim.x*blockIdx.x;
  for (int loop=index; loop<jobCount; loop+=gridDim.x*blockDim.x) {
    // if (index >= jobCount) return;  // 超出范围
    index = loop;
    unsigned int line[2048] = {0xFFFFFFFF};  // 保存结果
    for (int i=0; i<2048; i++) {
...
void clustering(Option &option, Data &data, Bench &bench) {
    ...
    while (true) {  // 聚类
        ...
        updatJobs(bench.jobList, bench.jobCount);  // 更新任务 ok
        checkError();
        if (bench.jobCount > 0) {  // magic ok
            kernel_magicFilter<<<(bench.jobCount+127)/128, 128>>>(data.magic,
            data.wordCutoff, bench.jobList, bench.jobCount, bench.representative);
        }
        cudaDeviceSynchronize();  // 同步数据
        checkError();
        //----比对工作----//
        ...
        if (bench.jobCount > 0) {  // 动态规划 ok
            // kernel_dynamic<<<(bench.jobCount+127)/128, 128>>>(option.threshold,
            kernel_dynamic2<<<64, 128>>>(option.threshold,
            data.lengths, data.offsets, data.gaps, data.compressed,
            data.baseCutoff, bench.cluster, bench.jobList, bench.jobCount,
