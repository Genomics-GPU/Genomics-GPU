...
__global__ void __cdpassertion(int const iSize, int iDepth, int cblocks, int cthreads, cudaStream_t cstream, GPUData data, char* centerSeq, char* seqs, int centerSeqLength, int *seqsSize, cudaPitchedPtr matrix3DPtr, short *space, short *spaceForOther, int maxLength, int workload)
{
     	int tid = threadIdx.x;
...
void cuda_msa(int offset, GPUData data, string centerSeq, int maxLength, short *space, short *spaceForOther) {
	...
	dim3 block (blocksize, 1);
        ...
        __cdpassertion<<<block,grid>>>(block.x, 1, BLOCKS, THREADS, data.stream, data, data.d_centerSeq, data.d_seqs, centerSeq.size(), data.d_seqsSize, data.matrix3DPtr, data.d_space, data.d_spaceForOther, maxLength, workload);
